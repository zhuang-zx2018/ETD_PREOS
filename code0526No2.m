% function [x3] = SVLE_CES_corrected(PB)
...
%Arbitrary value of 10000 Pa = 0.1 bar for triple point pressure
...
while abs(x(3)-x3) > 10^(-6)
    ...
    % VAPOR CALCULATION
    % to calculate fugacity coefficients in the vapor phase
    bV = bi*y';
    ...
    % LIQUID CALCULATION
    % to calculate fugacity coefficients in the liquid phase
    bL = bi*x';
    % New mixing rule incorporating UNIFAC
    ulambda=0.36;
    Av=-0.623;
    ...
    fL(3) = PB * phiL(3);
    x3 = fS/ fL(3);
    x2 = (1+(g1new-g3new)*x3-g1new)/(g2new-g1new);
    x1 = 1-x3-x2;
end